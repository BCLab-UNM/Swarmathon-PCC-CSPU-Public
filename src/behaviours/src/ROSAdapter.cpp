/**
  original_disableUpdatingCenter
map_ave
  **/
#include <ros/ros.h>
...
const float heartbeat_publish_interval = 1;
...
Point locationSumup;
bool pause2wait = true;//true for initialization
char currentState = '0';
time_t collectionZoneSeenStartTime;
...
void behaviourStateMachine(const ros::TimerEvent&)
{
    ...
    if (!initilized)
    {
        ...
        if (timerTimeElapsed > startDelayInSeconds)
        {
            ...
            //set currentLocation
            logicController.SetPositionData(locationSumup);
            logicController.SetVelocityData(linearVelocity, angularVelocity);

            //set center location
            centerMap.x = locationSumup.x + normal_offSet_dst_centerUpdate*cos(locationSumup.theta);
            centerMap.y = locationSumup.y + normal_offSet_dst_centerUpdate*sin(locationSumup.theta);
...
void targetHandler(const apriltags_ros::AprilTagDetectionArray::ConstPtr& message) {
    ...
    if (message->detections.size() > 0) {
        ...
        for (int i = 0; i < message->detections.size(); i++) {
            ...
            tags.push_back(loc);
            ////!!!can not be >= but either one
            collectionZoneSeen = loc.getID()==256 && time(0) - collectionZoneSeenStartTime > ave_interval?true:false;
        }
...
//        sumUpLocation(currentLocation,locationSumup);
        ...
        curr_loc.theta = currentLocation.theta;
